export function binRows(row, darkArr, lightArr) {
  if (row[1].length > 10) {
    alert(`Time is incorrectly formatted: ${row[1]}`);
  }

  const time = row[1].trim();
  row = row.slice(5, 19);
  //TODO add light/dark time range as inputs, default to 6 and 18
  if (time >= '06:00:00' && time < '18:00:00') {
    ...
    if (lightArr.length > 0) {
      ...
      lightArr = lightArr.concat(row);
    }
  } else {
    if (darkArr.length > 0) {
      darkArr = darkArr.map((o, i) => o + row[i]);
    } else {
      darkArr = darkArr.concat(row);
    }
  }

  return [darkArr, lightArr];
}
